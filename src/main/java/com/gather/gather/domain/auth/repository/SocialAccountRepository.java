...
public interface SocialAccountRepository extends JpaRepository<SocialAccount, Long> {
    ...
    @Query("select sa from SocialAccount sa join fetch sa.user u where u.status = :status")
    List<SocialAccount> findByUserStatus(UserStatus status, Pageable pageable);
}

...
public class AccountLoginTypeResolver {
    ...
    /** 계정 복구·비밀번호 재설정처럼 ACTIVE 계정만 대상으로 하는 흐름에서 사용한다. ACTIVE가 아니면 credential을 보지 않고 빈 값이다. */
    public Optional<AccountLoginType> resolveForActiveAccount(User user) {
        if (user.getStatus() != UserStatus.ACTIVE) {
            return Optional.empty();
        }

        return resolveCredentialTypeIgnoringStatus(user);
    }
     ...
     */
    public Optional<AccountLoginType> resolveCredentialTypeIgnoringStatus(User user) {

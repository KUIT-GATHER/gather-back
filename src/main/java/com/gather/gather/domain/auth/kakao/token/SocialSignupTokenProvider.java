 ...
 * 걸린다.
 *
 * <p>일회용 처리는 하지 않는다(jti를 저장하지 않음). 재사용은 15분 만료와 {@code (provider, provider_user_id)} 유니크 제약, 가입 직전
 * 재조회로 막는다.
 */
@Component
public class SocialSignupTokenProvider {
    ...
    public String createSignupToken(SocialProvider provider, String providerUserId) {
        ...
        return Jwts.builder()
                ...
                .add(AUDIENCE)
                .and()
                .subject(toSubject(provider, providerUserId))
    ...
    /**
     * 가입용 토큰을 검증하고 provider와 카카오 회원번호를 추출한다.
     *
    ...
    public SocialSignupTokenPayload parseSignupToken(String token) {
        ...
        return new SocialSignupTokenPayload(provider, parseProviderUserId(claims, provider));
    }
    ...
    private String parseProviderUserId(Claims claims, SocialProvider provider) {
        String subject = claims.getSubject();
        String expectedPrefix = subjectPrefix(provider);
        if (subject == null || !subject.startsWith(expectedPrefix)) {
            throw new BusinessException(ErrorCode.SIGNUP_TOKEN_INVALID);
        }
        String providerUserId = subject.substring(expectedPrefix.length());
    ...
    private String toSubject(SocialProvider provider, String providerUserId) {
        return subjectPrefix(provider) + providerUserId;
    }

...
public interface SocialSignupSessionRepository extends JpaRepository<SocialSignupSession, Long> {
    ...
    @Query(
            """
            UPDATE SocialSignupSession session
            SET session.status = :cancelledStatus,
            ...
            """)
    int cancelPendingByIdentity(
            @Param("provider") SocialProvider provider,
            @Param("providerUserKey") String providerUserKey,
            @Param("pendingStatus") SocialSignupSessionStatus pendingStatus,
            @Param("cancelledStatus") SocialSignupSessionStatus cancelledStatus,
            @Param("cancelledAt") LocalDateTime cancelledAt);

    default int cancelPendingByIdentity(
            SocialProvider provider, String providerUserKey, LocalDateTime cancelledAt) {
        return cancelPendingByIdentity(
                provider,
                providerUserKey,
                SocialSignupSessionStatus.PENDING,
                SocialSignupSessionStatus.CANCELLED,
                cancelledAt);
    }
}

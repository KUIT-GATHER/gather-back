 ...
 * 카카오 추가정보 가입 요청.
 *
 * <p>{@code SignupRequest}를 재사용할 수 없어 필드를 복제했다. 그쪽의 email·password·passwordConfirm이
 * {@code @NotBlank}라 이메일·비밀번호를 받지 않는 카카오 가입 요청으로는 검증을 통과할 수 없다. 나머지 필드는 이름과 검증 애노테이션을 그대로 맞춘다.
 */
@Schema(description = "카카오 추가정보 가입 요청")

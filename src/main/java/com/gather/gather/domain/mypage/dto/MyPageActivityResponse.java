...
public record MyPageActivityResponse(
        ...
        @Schema(description = "모임 ID (MEETING_RECRUIT인 경우만 존재)", nullable = true, example = "15")
                Long meetingId,
        @Schema(
                        description = "모집공고 게시글 ID (MEETING_RECRUIT인 경우만 존재)",
                        nullable = true,
                        example = "82")
                Long postId,
        ...
        @Schema(description = "활동 종료일", nullable = true, example = "2026-08-18")
    ...
    /**
     * 일반 1365/VMS 봉사 참여 일정. 2026-08 정책 변경: 날짜 source가 공고 전체 활동기간에서 사용자 개인 {@code
     * PostingParticipation.participationStartDate/EndDate}로 바뀌었다. 개인 일정이 없는(정책 변경 이전) 기존 데이터만 공고 전체
     * 활동기간으로 fallback한다 — 신규 정책 적용 이후 생성되는 참여는 항상 개인 일정을 가지므로 fallback 경로를 타지 않는다.
     */

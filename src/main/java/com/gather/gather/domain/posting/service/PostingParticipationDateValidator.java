 ...
 *
 * <p>이번 정책에서는 {@code Posting.actWkdy}를 신청 날짜 검증에 사용하지 않는다 — 1365는 raw string을 그대로 저장하고 VMS
 * 동기화에는 대응 필드 자체가 없어 소스 간 정합성이 부족하기 때문이다(devplan 정책 결정 참고). 검증은 공고의 전체 활동기간(actStartDate ~
 * actEndDate, 없으면 activityDate로 대체)과 오늘 날짜만 기준으로 한다.
 */
final class PostingParticipationDateValidator {
    ...
    static void validate(Posting posting, LocalDate startDate, LocalDate endDate, LocalDate today) {
        ...
        LocalDate postingStart =
                posting.getActStartDate() != null ? posting.getActStartDate() : posting.getActivityDate();
        LocalDate postingEnd =
                posting.getActEndDate() != null ? posting.getActEndDate() : posting.getActivityDate();

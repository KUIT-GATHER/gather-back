 ...
 *
 * <p>2026-08 정책 변경: 활동종료 여부는 더 이상 공고 전체 {@code Posting.actEndDate}가 아니라 사용자 개인
 * {@code PostingParticipation.participationEndDate} 기준으로 계산한다. 개인 일정이 없는(정책 변경 이전) 기존 참여만
 * 공고 전체 종료일로 fallback한다.
 */
public enum PostingParticipationAction {
     ...
     * 조회 3곳 모두 이 메서드 하나로 판정 기준을 통일한다 — 서로 다른 기준을 쓰면 "상세에서는 완료 가능인데 마이페이지에서는 아니다" 같은 불일치가
     * 생긴다.
     */

...
class SocialSignupSessionPersistenceServiceTest {
    ...
    @Mock private SocialAccount socialAccount;
    ...
    void saveNewAttemptChecksBlockAfterInsertHasJoinedIdentitySerialization() {
        ...
        SocialSignupSessionPersistenceService service =
                new SocialSignupSessionPersistenceService(
                        sessionRepository, socialAccountRepository, rejoinBlockService);
        ...
        InOrder order = inOrder(sessionRepository, socialAccountRepository, rejoinBlockService);
    ...
    @Test
    void saveNewAttemptRejectsAccountCreatedAfterTheOuterPrecheck() {
        SocialSignupSession session =
                SocialSignupSession.createKakao(
                        "b".repeat(64),
                        IDENTIFIER,
                        ...
                        NOW.plusMinutes(15),
                        NOW);
        when(socialAccountRepository.findByProviderAndProviderUserKeyForUpdate(
                        session.getProvider(), session.getProviderUserKey()))
                .thenReturn(Optional.of(socialAccount));
        when(socialAccount.isLinked()).thenReturn(true);
        SocialSignupSessionPersistenceService service =
                new SocialSignupSessionPersistenceService(
                        sessionRepository, socialAccountRepository, rejoinBlockService);
